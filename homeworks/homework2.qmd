---
title: Session 2 - OLS regressions
toc: true
toc-depth: 3
editor: 
  markdown: 
...
Download the European Social Survey wave 11 dataset (called ESS11) from <"https://github.com/paulservais-1/methods_policyevaluation/raw/main/dataexos/ESS11_data/ESS11.csv">, using package `rio` and `import()`. Import the dataset in R. In this exercise, we are interested in understanding what explains trust in parliament (trstprl). The main factors considered are Education level (eduyrs), Political interest (polintr), Media Use (tvtot, netusoft);

1. Have a look at the summary statistics of all the variable mentioned before (trstprl, eduyrs, polintr, netusoft).
2. In point 2, you see in the descriptive statistics that some variables have a maximum of 99. However, if you check the [codebook](https://github.com/paulservais-1/methods_policyevaluation/raw/main/dataexos/ESS11_data/ESS11_codebook.html), you will see that most of the questions are 10-point scale. Check the codebook to see if the 99 value is a code corresponding to missing values for example. If necessary, clean the data.

3. Check the `summary()` once you have cleaned the data.

4. Run a simple linear regression with trstprl as the dependent variable and eduyrs as independent
variable.
  a. What is the value of the coefficient of edurs ?
  b. Is the coefficient statistically different from 0?

5. From point 2, add the polintr and netusoft variables in the model.
  a. Is there a variable that significantly influences trust ? Which one?
  6. Present a clean regression table using the stargazer package.
...
#1.1
summary(ESS_11 %>% select("trstprl","eduyrs","polintr","netusoft"))
#1.2
...
summary(reg1)
#1.4
reg2<-lm(trstprl ~ eduyrs + polintr + netusoft, data_set)
summary(reg2)
#1.5
library(stargazer)
...
You can find the recent survey's dataset on the internet at <https://www.chesdata.eu/s/1999-2024_CHES_dataset_meansV2-3k4l.csv>, to perform the following tasks :  

  1. Import the data in your local environment. Use the `rio` package use `install.packages()` if you haven't installed the packages yet) and library to import it directly from the internet. 
...
#Exercice 1
#1.1
ches <- import("https://www.chesdata.eu/s/1999-2024_CHES_dataset_meansV2-3k4l.csv")
#1.2
glimpse(ches)
#1.3
