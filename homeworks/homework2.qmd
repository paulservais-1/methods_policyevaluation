...
1. Using the rio packages by calling `library(rio)` (if not already installed in your environment, install the package first), import the data from the ESS (wave 11), located at <>
2. Using `colnames`, have a look at the names of your variables. 
2. Using function `unique`, have a look at the different years present in your data set
```{r}
library(rio)

vot_turnout <- import("/Users/paulservais/Library/CloudStorage/ProtonDrive-paservai@protonmail.com-folder/Website/methods_policyevaluation/dataexos/rain_turnout.dta")

unique(vot_turnout$year)
