...
# Add outlier (extreme Y, normal X)
outlier <- tibble(
  x = 5,
  y = 20
)

data_outlier <- bind_rows(data_base, outlier)

# Flag points
data_outlier <- data_outlier %>%
  mutate(type = ifelse(row_number() == n()+1, "Outlier", "Normal"))

# Plot
plot_outlier <- ggplot(data_outlier, aes(x, y)) +
  ...
  labs(
    title = "Effect of an Outlier on Regression",
    subtitle = "Red line: with outlier | Blue dashed line: without outlier",
    color = ""
  ) +
...
high_leverage <- tibble(
  x = 12,
  y = 2*12 + rnorm(1, 0, 1)
)

data_leverage <- bind_rows(data_base, high_leverage)

# Flag points
data_leverage <- data_leverage %>%
  mutate(type = ifelse(row_number() == n()+1, "High leverage", "Normal"))

# Plot
plot_leverage <- ggplot(data_leverage, aes(x, y)) +
  geom_point(aes(color = type), size = 3) +
  ...
  labs(
    title = "Effect of a High Leverage Point on Regression",
    subtitle = "Red line: with leverage point | Blue dashed line: without leverage point",
    color = ""
  ) +
...
data_influential <- bind_rows(data_base, influential) %>%
  mutate(type = ifelse(row_number() == n()+1, "Influential", "Normal"))

ggplot(data_influential, aes(x, y)) +
  geom_point(aes(color = type), size = 3) +
  geom_smooth(method = "lm", se = FALSE, color = "red", linewidth = 1.2) +
  geom_smooth(data = data_base, method = "lm", se = FALSE,
              color = "blue", linewidth = 1.2, linetype = "dashed") +
  scale_color_manual(values = c("black", "red")) +
  labs(
    title = "Influential Point (High Leverage + Outlier)",
    subtitle = "Largest impact on regression slope"
  ) +
  theme_minimal(base_size = 16)
```
# Interactions in linear regressions
...
data <- import("https://github.com/paulservais-1/methods_policyevaluation/raw/main/homeworks/data/schooltutorprog.csv") %>% filter(test_score > 0 & test_score <=100)
glimpse(data)
summary(data)
