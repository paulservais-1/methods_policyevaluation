...
match_model <- matchit(treatment ~ .,
                       data = data %>% filter(time == pre-treatment_period) %>% select(-c(outcome_var)),
                       method = "nearest",   # nearest neighbor matching
                       distance = "logit")   # use logistic propensity score

matched_ids <- match.data(match_model) %>% pull(id) %>% unique()
```
## Propensity score matching
