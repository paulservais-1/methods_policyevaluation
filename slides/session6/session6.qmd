...
## Causal estimate
- $\delta$ is your causal estimate
- It is a LATE !
- "The 1-unit variation in D caused by Z leads to a $\delta$ increase in Y"
...
3.  Is independent of unobserved confounders (exogeneity)
...
## An example
- Gavazza, Nardotto, and Valletti (2017) : [**Internet and Politics: Evidence from U.K. Local Elections and Local Government Policies**](https://www.qmul.ac.uk/sef/media/econ/images/documents/UKinternet.pdf)


## Instrumental variables
- Require very good institutional knowledge
- Weather (rainfall has often been used)
...
## Overall
- Correlation is not always meaningless
...
df <- import("https://github.com/paulservais-1/methods_policyevaluation/raw/main/data/turnout_internet.dta")

ols1 <- lm(turnout ~ internet, data = df)
summary(ols1)

ols_controls <- lm(turnout ~ internet + income + graduates + unemployment + urban, data = df)
summary(ols_controls)


# replace "instrument" with actual variable name in dataset
first_stage <- lm(internet ~ rain + income + graduates + unemployment + urban, data = df)
summary(first_stage)

df$internet_hat <- fitted(first_stage)

second_stage_manual <- lm(turnout ~ internet_hat + income + graduates + unemployment + urban, data = df)
summary(second_stage_manual)

iv_model <- ivreg(
  turnout ~ internet + income + graduates + unemployment + urban |
    rain + income + graduates + unemployment + urban,
  data = df
)
